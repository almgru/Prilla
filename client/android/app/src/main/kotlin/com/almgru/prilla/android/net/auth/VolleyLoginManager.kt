...
class VolleyLoginManager(
    private val context: Context, private val persistenceManager: PersistenceManager
) {
    ...
    private fun isLoginSuccessRedirect(response: NetworkResponse): Boolean {
        return response.headers?.get("Location") == "${persistenceManager.getServerUrl()}${
            context.getString(
                R.string.server_login_success_endpoint
            )
        }"
    }
    ...
    private fun isBadCredentialsRedirect(response: NetworkResponse): Boolean {
        return response.headers?.get("Location") == "${persistenceManager.getServerUrl()}${
            context.getString(
                R.string.server_login_failure_endpoint
            )
        }"
    }
